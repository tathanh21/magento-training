 ...
 * Class MassDelete
 */
class EditStatus2 extends \Magento\Backend\App\Action implements HttpPostActionInterface
{
    /**
...
//     $collection->addFieldToFilter('id',1);
        foreach ($id as $item) {
           ...
           $data->setData('status','2')->save();
//            $block->setStatus('2')->save();
            $count++;
        }

        $this->messageManager->addSuccessMessage(__('A total of %1 record(s) have been Set Status Draft.',$count));
